...
Version baseline: GTO 0.5.6-beta, GTOLib 26.7.4, GTCEu 26.7.3, and Forge 47.4.20. The current build target is `gtohjs-4.0-per3-for-gtocore-0.5.6-beta.jar`; the preceding clean-build baseline is `gtohjs-4.0-per2-for-gtocore-0.5.6-beta.jar`. Core resources are read-only; GTOLib is used only for read-only ABI audits; this documentation describes only GTOHJS development strategy.
